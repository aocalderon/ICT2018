...
X = train_file.create_dataset('X', (NUMBER_OF_PATCHES*AUGMENTORS,ratio,ratio,4), dtype='float32')      
...
while i < NUMBER_OF_PATCHES*AUGMENTORS:
    ...
    filename = os.path.join(data_input, "PA_{}.tif".format(n))
    pa = rasterio.open(filename, 'r+')
    image = pa.read().astype('float32')
    b = image[0]
    imin, imax = np.percentile(b, [1,99])
    b[b > imax] = imax
    b[b < imin] = imin
