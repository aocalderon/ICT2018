...
img_size = 64 * _image_scale_multiplier
stride = 32 * _image_scale_multiplier
...
input_path      = "/opt/EuroSAT/Test/train/" # r"input_images/"
validation_path = "/opt/EuroSAT/Test/vallid/" # r"D:\Yue\Documents\Datasets\MSCOCO\val\valset\\" # r"val_images/"

validation_set5_path = validation_path + "set5/"
validation_set14_path = validation_path + "set14/"

base_dataset_dir = "/opt/EuroSAT/Test/"

output_path = base_dataset_dir + "train/"
validation_output_path = base_dataset_dir + "valid/"

if not os.path.exists(output_path):
    os.makedirs(output_path)
    
def transform_images(directory, output_directory, size, scaling_factor=4, max_nb_images=-1):
    hr_size = size * scaling_factor
    lr_size = int(hr_size / scaling_factor)
    index = 1
    ...
    for file in files:
        ...
        ip = imresize(img, (hr_size, hr_size))
        ip = imresize(ip, (size, size))
        plt.imshow(ip)
        imsave(output_directory + "X/" + "%d.png" % (index), ip)
        op = imresize(img, (lr_size, lr_size), interp='bicubic')
        op = imresize(op, (size, size))
        plt.imshow(op)
        imsave(output_directory + "y/" + "%d.png" % (index), op)
